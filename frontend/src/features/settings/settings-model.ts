...
export const settingsSchema = z.object({
  ...
  media: z.object({
    ...
    retention: durationSchema.refine((value) => durationSeconds(value) >= 3_600 && durationSeconds(value) <= 31_536_000),
...
export function toSettingsForm(config: SettingsConfigDTO): SettingsForm {
  return {
    ...
    media: {
      ...
      retention: parseDuration(config.media.retention),
    },
    frontend: {
...
export function toSettingsDTO(config: SettingsForm): SettingsConfigDTO {
  return {
    ...
    media: {
      ...
      retention: formatDuration(config.media.retention),
    },
    frontend: {
