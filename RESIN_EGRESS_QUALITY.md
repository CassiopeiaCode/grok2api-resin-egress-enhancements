...
本文说明本分支如何把 Resin 粘性代理池接入 Grok2API，并通过独立的鹈鹕质量守护服务维护 Build 好出口池。生产 Build 请求使用池内 Resin 完整用户名；token 速度、响应头和首 token 静默不再直接修改鹈鹕池。

## 鹈鹕好出口池

系统最多维护 3 个通过鹈鹕提示词验证的 Resin 用户名。用户名本身就是 Resin 的粘性出口身份，与 Grok 账号 ID 解耦：探测时可以随机使用一个正常 Build 账号，生产时再由账号稳定哈希选择池内用户名。

池成员必须同时满足：

- 原始提示词为 `画一个鹈鹕骑自行车的svg`；
- SVG 提取成功且 XML 可解析；
- 固定版本分类器标签为 `good`；
- 分类置信度至少 `0.60`。

每个成员每 10 分钟复检一次。复检结果不是 `good >= 0.60` 时立即移除；池少于 3 个时自动探索新的随机正常 Build 账号和新的 Resin 用户名。池为空时生产 Build 回退到账号自己的 Resin 身份。

生产请求不会因为 token 速度、HTTP 响应头超时或流式静默直接修改池；这些信号仍写入审计。Web/Console 原有响应头超时轮换逻辑保持不变。

鹈鹕守护服务位于仓库的 `pelican-egress-guard` 目录，由同一 Compose 部署。分类器使用固定模型快照，不在运行时从标注数据重新计算权重。模型快照包含特征版本、`means`、`scales`、训练向量、标签和 SHA-256；原始标注 SVG、管理员凭据、客户端 Key 和 Resin token 不提交到 GitHub。
